...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        // recap on flow...
        // tool report into datacollector 
        // avro format publish to Kafka
        // Report Parser from Kafka as a Rust object / blob of json objects
        // Report Parser enrich the data
        // Parse code into dependency events into Topics
        // into kakfa
        if let Some(Ok(message)) = messages.next().await {
            if let Some(body) = message.payload() {
                let reader = Reader::new(body)?;
                for value in reader {
                    // taking ToolReport as an avro object and turn it into a tool
                    // report rust struct
...
) -> Result<Vec<Vec<u8>>, Box<dyn Error>> 
{
    ...
    //note: could be match statement to check if yarn-audit
    /*
...
*/
    ...
    // TO DO need to replace json_file with &report output
    //let _lines = lines_from_file(json_file);
    //let yarn: serde_json::value::Value = serde_json::from_str(&_lines[1]).expect("JSON was not well-formatted");  

    // substitute yarn output toolreport
    // substitue block.get(0) to get the header from the yarn tool report

    
    /*  Bundler                              Yarn
        "Name":"actionpack"                  "module_name":"minimatch"
        "Advisory": "CVE-2020-8185"          "cves":"CVE-2017-16042"


        pull advisory_id from cves field above and clean up code below
        https://github.com/simplybusiness/Kiln/blob/main/kiln_lib/src/dependency_event.rs
        use "cves" field above and clean up code below
        use serde json (see kiln.lib)
        serde.json::value lets me operate on json
        let cvss = vulns.get(&advisory_id.to_string()).unwrap_or(&default_cvss);
        note: preserve the format below
        see this url for the data format
        https://github.com/simplybusiness/Kiln/blob/main/data-collector/docs/request-response.md
    */
        /*
         let mut event = DependencyEvent 
        {
            event_version: kiln_lib::tool_report::EventVersion::try_from("1".to_string()).unwrap(),
            event_id: EventID::try_from(Uuid::new_v4().to_hyphenated().to_string())?,
            parent_event_id: report.event_id.clone(),
            //application_name: kiln_lib::tool_report::ApplicationName::try_from(yarn["data"]["advisory"]["module_name"].to_string()).unwrap(),
            git_branch: report.git_branch.clone(),
            git_commit_hash: report.git_commit_hash.clone(),
            timestamp: Timestamp::try_from(report.end_time.to_string())?,
            suppressed: false,
...
// try writing unit test for yarn audit
// parse out one finding.
// cargo test
// put json in line
