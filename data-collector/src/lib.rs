...
use actix_http::{error::PayloadError, h1::Payload as HttpPayload};
use chrono::{SecondsFormat, Utc};
use futures::{future::{ok, Ready}, io::copy};
use futures::stream::{Stream, StreamExt};
use kiln_lib::validation::ValidationError;
use serde::{Deserialize, Serialize};
use slog::{error, info, o, Logger};
use slog_derive::SerdeValue;
use std::{borrow::{BorrowMut, ToOwned}, ops::{Deref, DerefMut}};
...
impl Stream for CopyStream {
    ...
    fn poll_next(
        ...
        match input.deref_mut() {
            ...
            actix_http::Payload::H2(p) => {todo!()},
            actix_http::Payload::Stream(p) => {todo!()},
        }
    }
}
    ...
    B: MessageBody + 'static + Unpin,
{
    ...
    fn call(&self, mut req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            //println!("{:?}", req_body_bytes);
            //let req_body = String::from_utf8(req_body_bytes.clone().to_vec()).unwrap();

            //let (mut http_payload_sender, http_payload) = HttpPayload::create(true);

            //http_payload_sender.feed_data(req_body_bytes.clone());
            //println!("{:?}", http_payload);
            ...
            let boxed_payload: Pin<Box<dyn Stream<Item=Result<bytes::Bytes, PayloadError>>>> = copy_payload.clone().into();
...
mod tests {
    ...
    async fn request_logger_passes_through_request_body() {
        ...
        let app = actix_web::test::init_service(
            actix_web::App::new()
                .wrap(sut)
                .service(actix_web::web::resource("/test").to(move |actual_body: actix_web::web::Bytes| async move {
        ...
        let req = TestRequest::with_uri("/test").set_payload(expected_body).to_request();
